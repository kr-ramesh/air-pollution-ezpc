...
int main(int argc, char **argv) {
  ...
  //initializing numbers for testing
  ...
  //Sample range of scales for testing
  for(int i=-10;i<=2;i++)
  {
  cout<<endl<<endl<<"Scale Factor"<<i;
  scale_factor[0]=i;
  ...
  //Evaluates for which of the numbers (exp+scale-23) is greater than 0
  is_greater_than_23=fix->GT(check, 23);
  
  //Evaluates for which of the numbers (exp+scale) is greater than -1 (If true, the number approximates to 0) 
  ...
  //Represents the value of m*2^(exp+scale). v0 obtained from the lookup table. Condition: if exp+scale-23>0, right shift v0 by 23 before multiplying it with 'a'.
