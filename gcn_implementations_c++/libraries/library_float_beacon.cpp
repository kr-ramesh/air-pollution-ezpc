...
void updateWeightsAdam_thread(
	...
	FPArray weight = fpopArr[tid]->input(tid&1?3-__party:__party, chunk, in_s, in_z, in_m, in_e, m_bits, e_bits) ;
	FPArray der = fpopArr[tid]->input(tid&1?3-__party:__party, chunk, der_s, der_z, der_m, der_e, m_bits, e_bits) ;
    FPArray m_t = fpopArr[tid]->input(tid&1?3-__party:__party, chunk, 0, m_bits, e_bits) ;
	FPArray v_t = fpopArr[tid]->input(tid&1?3-__party:__party, chunk, 0, m_bits, e_bits) ;

    FPArray muller = fpopArr[tid]->input<float>(PUBLIC, chunk, lr, m_bits, e_bits) ;
    FPArray b1 = fpopArr[tid]->input<float>(PUBLIC, chunk, beta1, m_bits, e_bits) ;
    FPArray b2 = fpopArr[tid]->input<float>(PUBLIC, chunk, beta2, m_bits, e_bits) ;
	FPArray e = fpopArr[tid]->input<float>(PUBLIC, chunk, eps, m_bits, e_bits) ;
	FPArray sub1=fpopArr[tid]->input<float>(PUBLIC, chunk, 1, m_bits, e_bits);

    m_t=fpopArr[tid]->add(fpopArr[tid]->mul(der, b1), fpopArr[tid]->mul(der, fpopArr[tid]->sub(sub1, b1)));
	v_t=fpopArr[tid]->add(fpopArr[tid]->mul(der, b2), fpopArr[tid]->mul(fpopArr[tid]->mul(der, der), fpopArr[tid]->sub(sub1, b2)));
	for(uint32_t i=0; i<t; i++){
		b1=fpopArr[tid]->mul(b1, b1);
		b2=fpopArr[tid]->mul(b2, b2);
		}
	
	m_t=fpopArr[tid]->div(m_t, fpopArr[tid]->sub(sub1, b1));
	v_t=fpopArr[tid]->div(v_t, fpopArr[tid]->sub(sub1, b2));
...
void updateWeightsAdam(int32_t sz, int32_t t, float lr, float beta1, float beta2, float eps, vector<FPArray>& inArr, vector<FPArray>& derArr)
{
	int m_bits, e_bits ;
	m_bits = inArr[0].m_bits ;
	e_bits = inArr[0].e_bits ;
...
void Sigmoid_thread(
	...
	FPArray in_flat = fpopArr[tid]->input(WHICHPARTY, sz, in_s, in_z, in_m, in_e, m_bits, e_bits) ;
	FPArray out_flat = fpmathArr[tid]->sigmoid(in_flat) ;
...
void Tanh_thread(
	...
	FPArray in_flat = fpopArr[tid]->input(WHICHPARTY, sz, in_s, in_z, in_m, in_e, m_bits, e_bits) ;
	FPArray out_flat = fpmathArr[tid]->tanh(in_flat) ;
