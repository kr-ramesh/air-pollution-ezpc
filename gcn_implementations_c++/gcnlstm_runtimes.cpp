...
cout<<"Updates"<<endl;
updateWeightsAdam2(gcn1dim3, gcn2dim3, total_t, 0.01, 0.9, 0.99, 1e-7, kernelarr, dkernelarr, m1, v1);
updateWeightsAdam2(totaltimesteps, gcn2dim3, total_t, 0.01, 0.9, 0.99, 1e-7, kernelarr2, dkernelarr2, m2, v2);
updateWeightsAdam2(d2, d2, total_t, 0.01, 0.9, 0.99, 1e-7, dLastnodes1, lastnodes, m3, v3);
updateWeightsAdam2(d2, d2, total_t, 0.01, 0.9, 0.99, 1e-7, dLastnodes2, lastnodes2, m4, v4);
updateWeightsAdam(d2, total_t, 0.01, 0.9, 0.99, 1e-7, dbias1, bias1, m5, v5);
updateWeightsAdam(d2, total_t, 0.01, 0.9, 0.99, 1e-7, dbias2, bias2, v6, v6);
updateWeightsAdam2(d2, hdim4, total_t, 0.01, 0.9, 0.99, 1e-7, Fil, DFilSum, m7, v7);
updateWeightsAdam2(hdim, hdim4, total_t, 0.01, 0.9, 0.99, 1e-7, RecFil, DRecFilSum, m8, v8);
updateWeightsAdam(hdim4, total_t, 0.01, 0.9, 0.99, 1e-7, LSTMBias, DBiasSum, m9, v9);
updateWeightsAdam2(hdim, total_t, d2, 0.01, 0.9, 0.99, 1e-7, layer1W, layer1WDerReshaped, m10, v10);
updateWeightsAdam(d2, total_t, 0.01, 0.9, 0.99, 1e-7, layer1b, layer1bDer, m11, v11);
}
...
/*auto inp = make_vector_float(ALICE, 1, 270, 3) ;
...
auto hidden = make_vector_float(ALICE, 1, 4) ;
...
auto cell = make_vector_float(ALICE, 1, 4) ;
...
for(uint32_t iterations=0; iterations<1; iterations++)
{
    ...
    for (uint32_t ind = 0; ind < num_samples; ind=ind+d1){
        ...
        for (uint32_t batch_assign = ind; batch_assign < ind+num_assign; batch_assign++){
        AssignSampleFromData3D(d1, d2, d3, batch_assign, inp, inputpoint);
        AssignSampleFromData2D(d1, d2, batch_assign, labs, labels);
        }
        cout<<"FORWARD"<<endl;
