...
void updateWeightsAdam(int32_t s, float lr, int32_t t, float beta_1, float beta_2, float e, auto& bias, auto& der) {
    float m_t, v_t, m_cap, v_cap, deriv;
    for (uint32_t i1=0; i1<s; i1++)
    {
    m_t=beta_1*der[i1]+(1-beta_1)*der[i1];
    v_t=beta_2*der[i1]+(1-beta_2)*der[i1];
    m_cap=m_t/(1-pow(beta_1, t));
    v_cap=v_t/(1-pow(beta_2, t));
    deriv=m_cap/(sqrt(v_cap)+e);
    bias[i1] = bias[i1] - (lr*deriv) ;
    }
}

void updateWeights2Adam(int32_t s1, int32_t s2, float lr, int32_t t, float beta_1, float beta_2, float e, auto& weight, auto& der){
    float m_t, v_t, m_cap, v_cap, deriv;
    for (uint32_t i1 = 0; i1 < s1; i1++)
    {
        for (uint32_t i2 = 0; i2 < s2; i2++)
        {
            m_t=beta_1*der[i1][i2]+(1-beta_1)*der[i1][i2];
            v_t=beta_2*der[i1][i2]+(1-beta_2)*der[i1][i2];
            m_cap=m_t/(1-pow(beta_1, t));
            v_cap=v_t/(1-pow(beta_2, t));
            deriv=m_cap/(sqrt(v_cap)+e);
            weight[i1][i2] = weight[i1][i2] - (lr*der[i1][i2]) ; 
...
updateWeights2Adam(gcn1dim3, gcn2dim3, 0.01, adam_t, 0.9, 0.999, 1e-7, kernelarr, dkernelarr);
updateWeights2Adam(totaltimesteps, gcn2dim3, 0.01, adam_t, 0.9, 0.999, 1e-7, kernelarr2, dkernelarr2);
updateWeights2Adam(d2, d2, 0.01, adam_t, 0.9, 0.999, 1e-7, dLastnodes1, lastnodes);
updateWeights2Adam(d2, d2, 0.01, adam_t, 0.9, 0.999, 1e-7, dLastnodes2, lastnodes2);
updateWeightsAdam(d2, 0.01, adam_t, 0.9, 0.999, 1e-7, dbias1, bias1);
updateWeightsAdam(d2, 0.01, adam_t, 0.9, 0.999, 1e-7, dbias2, bias2);
updateWeights2Adam(d2, hdim4, 0.01, adam_t, 0.9, 0.999, 1e-7, Fil, DFilSum);
updateWeights2Adam(hdim, hdim4, 0.01, adam_t, 0.9, 0.999, 1e-7, RecFil, DRecFilSum);
updateWeightsAdam(hdim4, 0.01, adam_t, 0.9, 0.999, 1e-7, LSTMBias, DBiasSum);
updateWeights2Adam(hdim, d2, 0.01, adam_t, 0.9, 0.999, 1e-7, layer1W, layer1WDerReshaped);
updateWeightsAdam(d2, 0.01, adam_t, 0.9, 0.999, 1e-7, layer1b, layer1bDer);
}
...
for(uint32_t iterations=0; iterations<1; iterations++)
{
    ...
    adam_count+=1;
    backward(d1, d2, d3, 4, 4, gatesdim, 4, dense, bias4, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt, FullXt, finaloutput, labels, k, reck, lstmbias, neight2, kernel2, feat2, A2, neigh1, kernel1, feat1, A2, bias1, bias2, reluoutputs1, reluoutputs2, adam_count);
}
