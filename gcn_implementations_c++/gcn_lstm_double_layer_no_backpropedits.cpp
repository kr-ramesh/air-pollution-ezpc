...
void computeLoss(int32_t d1, int32_t d2, auto& labs, auto& predict, auto& losses)
{
    float sum=0;
...
void ReluCheck(int32_t d1, int32_t d2, int32_t d3, auto& arr1, auto& arr2){
    for (uint32_t i0 = 0; i0 < d1; i0++){
...
void FixedAdjacencyGraph(int32_t d1, int32_t d2, int32_t d3, int32_t d4, auto& features, auto& lastnodes, auto& kernelarr, auto& bias, auto& finalarr, auto& neighbours){
...
void LSTMStep(int32_t t, int32_t inputdim, int32_t hiddendim, int32_t dim1, int32_t dim3, auto& Inputs, auto& HInp, auto& CellStates, auto& Filter, auto& RecurrentFilter, auto& Bias, auto& FullHt, auto& FullIt, auto& FullFt, auto& FullGt, auto& FullOt, auto& FullCt){
...
void LSTM3D(int32_t numunits, int32_t idim, int32_t hdim, int32_t d1, int32_t d3, auto& AllInputs, auto& HState, auto& CellState, auto& Fil, auto& RecFil, auto& biasunit, auto& FullHt, auto& FullIt, auto& FullFt, auto& FullGt, auto& FullOt, auto& FullCt){
...
LSTMStep(iter, idim, hdim, d1, d3, InputExample, HState, CellState, Fil, RecFil, biasunit, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt);
}
}

void LSTM3Dv2(int32_t numunits, int32_t idim, int32_t hdim, int32_t d1, int32_t d3, auto& AllInputs, auto& HState, auto& CellState, auto& Fil, auto& RecFil, auto& biasunit, auto& FullHt, auto& FullIt, auto& FullFt, auto& FullGt, auto& FullOt, auto& FullCt){
auto InputExample = make_vector<float>(1, idim) ;

for (uint32_t iter = 0; iter < numunits; iter++){
for (uint32_t l = 0; l < idim; l++){
InputExample[0][l] = AllInputs[iter+1][0][l] ;
}
LSTMStep(iter, idim, hdim, d1, d3, InputExample, HState, CellState, Fil, RecFil, biasunit, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt);
}
}

void forward(int32_t d1, int32_t d2, int32_t d3, int32_t d4, auto& features, auto& lastnodes, auto& kernelarr, auto& gcnbias, auto& lastnodes2, auto& kernelarr2, auto& gcnbias2, int32_t unitstotal, int32_t idim, int32_t hdim, int32_t dim1, int32_t dim3, auto& hstates, auto& cellstates, auto& lstmkernel, auto& reclstmkernel, auto& lstmbias, auto& lstmkernel2, auto& reclstmkernel2, auto& lstmbias2, auto& denselayer, auto& bias4, auto& FullHt, auto& FullIt, auto& FullFt, auto& FullGt, auto& FullOt, auto& FullCt, auto& FullXt, auto& neight2, auto& feat2, auto& neigh1, auto& feat1, auto& finaloutput){
...
auto hstatescopy = make_vector<float>(d1, d4) ;

auto cellstatescopy = make_vector<float>(d1, d4) ;

AssignCopy2D(d1, d4, hstates, hstatescopy);
AssignCopy2D(d1, d4, cellstates, cellstatescopy);
...
LSTM3D(unitstotal, idim, hdim, dim1, dim3, finalarr3, hstates, cellstates, lstmkernel, reclstmkernel, lstmbias, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt);
Assign3(d1, hdim, d4, hstates, FullHt);


//Assign finalarr3 to fullxt1 and outputs from this lstm to fullxt2
//change to fullht2, etc
LSTM3Dv2(unitstotal, d4, hdim, dim1, dim3, FullHt, hstatescopy, cellstatescopy, lstmkernel2, reclstmkernel2, lstmbias2, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt);


MatMul(dim1, hdim, d2, hstatescopy, denselayer, finaloutput);
GemmAdd(dim1, d2, finaloutput, bias4, finaloutput);
Sigmoid2(dim1, d2, finaloutput, finaloutput);
for (uint32_t i = 0; i < d2; i++)
cout<<finaloutput[0][i]<<" ";
...
auto inp = make_vector<float>(d1, d2, d3) ;
...
auto k2 = make_vector<float>(d4, gatesdim) ;
...
for (uint32_t i1 = 0; i1 < gatesdim; i1++){
cin >> __tmp_in_k[0];
k2[i0][i1] = __tmp_in_k[0] ;

}
}
delete[] __tmp_in_k ;

auto reck2 = make_vector<float>(d4, gatesdim) ;

cout << ("Input reck2:") << endl ;


for (uint32_t i0 = 0; i0 < d4; i0++){
for (uint32_t i1 = 0; i1 < gatesdim; i1++){
cin >> __tmp_in_reck[0];
reck2[i0][i1] = __tmp_in_reck[0] ;

}
}
delete[] __tmp_in_reck ;

auto lstmbias2 = make_vector<float>(gatesdim) ;

cout << ("Input lstmbias2:") << endl ;

for (uint32_t i0 = 0; i0 < gatesdim; i0++){
cin >> __tmp_in_lstmbias[0];
lstmbias2[i0] = __tmp_in_lstmbias[0] ;
...
auto labs = make_vector<float>(d1, d2) ;
...
forward(d1, d2, d3, d4, inp, A1, kernel1, bias1, A2, kernel2, bias2, d4, d2, hdim, d1, gatesdim, hidden, cell, k, reck, lstmbias, k2, reck2, lstmbias2, dense, bias4, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt, FullXt, neight2, feat2, neigh1, feat1, finaloutput);
//computeLoss(1, 1989, labs, finaloutput, loss);
//backward(1, 1989, 12, 16, 16, 64, 16, dense, bias4, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt, FullXt, finaloutput, labs, k, reck, lstmbias, neight2, kernel2, feat2, A2, neigh1, kernel1, feat1, A2, bias1, bias2);
}
return 0;
}