...
Integer count(auto& x1, auto& x2, Integer threshold1){

Integer resultfinal = Integer(bitlen,  (int32_t)0, PUBLIC);
for (uint32_t i =  (int32_t)0; i <  (int32_t)270; i++){
/* Temporary variable for sub-expression on source location: (19,46-19,51) */
Bit __tac_var5 = x2[i];
/* Temporary variable for sub-expression on source location: (19,57-19,62) */
Integer __tac_var6 = x1[i];
/* Temporary variable for sub-expression on source location: (19,57-19,73) */
Bit __tac_var7 = __tac_var6.operator>(threshold1);
/* Temporary variable for sub-expression on source location: (19,77-19,78) */
Integer __tac_var8 = Integer(bitlen,  (int32_t)1, PUBLIC);
/* Temporary variable for sub-expression on source location: (19,81-19,82) */
Integer __tac_var9 = Integer(bitlen,  (int32_t)0, PUBLIC);
/* Temporary variable for sub-expression on source location: (19,56-19,82) */
Integer __tac_var10 =  If(__tac_var7, __tac_var8, __tac_var9);
/* Temporary variable for sub-expression on source location: (19,45-19,89) */
Integer __tac_var12 =  If(__tac_var5, __tac_var10, __tac_var9);
/* Temporary variable for sub-expression on source location: (19,30-19,90) */
resultfinal = resultfinal+(__tac_var12);
}
return resultfinal;
}
...
Integer __tac_var15 = grid1[i];
/* Temporary variable for sub-expression on source location: (41,21-41,29) */
Integer __tac_var16 = grid2[i];
grid[i] = __tac_var15.operator+(__tac_var16);
}
...
for (uint32_t i =  (int32_t)0; i <  (int32_t)270; i++){
/* Temporary variable for sub-expression on source location: (51,18-51,26) */
Bit __tac_var17 = bool1[i];
/* Temporary variable for sub-expression on source location: (51,27-51,35) */
Bit __tac_var18 = bool2[i];
clientbool[i] = __tac_var17.operator^(__tac_var18);
}
...
result=If(firstcheck, count(grid, clientbool, t1), Integer(bitlen, (int32_t)0, PUBLIC));
/* Temporary variable for sub-expression on source location: (61,17-61,31) */
Integer __tac_var19 = result.operator-(randnum);
cout << ("Value of __tac_var19:") << endl;
cout << (__tac_var19.reveal<int32_t>(ALICE)) << endl;
