...
cout<<"LSTM Matrix"<<endl;
auto lstm_start = clock_start();
float lstm_comm = __get_comm() ;
...
long long t_lstm = time_from(lstm_start);
float lstm_comm_end = __get_comm() ;

cout << "Total Time for LSTM Matmuls:\t" << t_lstm / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (lstm_comm_end - lstm_comm)/(1 << 20) <<endl;
...
lstm_start = clock_start();
lstm_comm = __get_comm() ;
...
t_lstm = time_from(lstm_start);
lstm_comm_end = __get_comm() ;

cout << "Total Time for LSTM Non-linear:\t" << t_lstm / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (lstm_comm_end - lstm_comm)/(1 << 20) ;
...
for (uint32_t iter = 0; iter < numunits; iter++) {
    ...
    cout<<"LSTM Step"<<endl;
    auto start_forward_step = clock_start();
    float comm_start_forward_step = __get_comm() ;
    ...
    long long t_forward_step = time_from(start_forward_step);
    float comm_end_forward_step = __get_comm() ;
    
    cout << "Total Time for LSTM Step:\t" << t_forward_step / (1000.0) << " ms" << endl;
    cout << "Total comms:\t" << (comm_end_forward_step - comm_start_forward_step)/(1 << 20) <<endl;
    }
}
...
auto start_forward_network = clock_start();
float comm_start_forward_network = __get_comm() ;
...
long long t_forward_network = time_from(start_forward_network);
float comm_end_forward_network = __get_comm() ;

cout << "Total Time for GCN 1:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20) ;

cout<<"GCN LSTM2"<<endl;

start_forward_network = clock_start();
comm_start_forward_network = __get_comm() ;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for GCN 2:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20) <<endl;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for LSTM:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20) <<endl ;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for Dense:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20) <<endl;
}
...
cout << "Total Time for Dense backprop:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20)<<endl;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for LSTM backprop:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20) <<endl;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for GCN 2 backprop:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20)<<endl ;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for GCN 1 backprop:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20)<<endl ;
...
t_forward_network = time_from(start_forward_network);
comm_end_forward_network = __get_comm() ;
cout << "Total Time for updates in backprop:\t" << t_forward_network / (1000.0) << " ms" << endl;
cout << "Total comms:\t" << (comm_end_forward_network - comm_start_forward_network)/(1 << 20)<<endl ;
}
...
for(uint32_t iterations=0; iterations<1; iterations++)
{
    ...
    for (uint32_t ind = 0; ind < num_samples; ind=ind+d1){
        ...
        */
        ...
        auto FullHt = make_vector_float(ALICE, (d4 + 1), num_assign, hdim) ;
        auto FullIt = make_vector_float(ALICE, d4, num_assign, hdim) ;
        auto FullFt = make_vector_float(ALICE, d4, num_assign, hdim) ;
        auto FullGt = make_vector_float(ALICE, d4, num_assign, hdim) ;
        auto FullOt = make_vector_float(ALICE, d4, num_assign, hdim) ;
        auto FullCt = make_vector_float(ALICE, (d4 + 1), num_assign, hdim) ;
        ...
        auto start_fw = clock_start();
        float comm_start_fw = __get_comm() ;
        cout<<"FORWARD"<<endl;
        ...
        long long t_fw = time_from(start_fw);
        float comm_end_fw = __get_comm() ;

        cout << "Total Time for Forward:\t" << t_fw / (1000.0) << " ms" << endl;
        cout << "Total comms:\t" << (comm_end_fw - comm_start_fw)/(1 << 20)<<endl ;
        //computeLoss(d1, d2, labels, finaloutput);

        start_fw = clock_start();
        comm_start_fw = __get_comm() ;
        cout<<"BACKWARD"<<endl;
        backward(num_assign, d2, d3, d4, hdim, gatesdim, d4, dense, bias4, FullHt, FullIt, FullFt, FullGt, FullOt, FullCt, FullXt, finaloutput, labels, k, reck, lstmbias, neight2, kernel2, feat2, A2, neigh1, kernel1, feat1, A2, bias1, bias2, reluoutputs1, reluoutputs2, m1, v1, m2, v2, m3, v3, m4, v4, m5, v5, m6, v6, m7, v7, m8, v8, m9, v9, m10, v10, m11, v11, total_timesteps);
        t_fw = time_from(start_fw);
        comm_end_fw = __get_comm() ;
        cout << "Total Time for Backward:\t" << t_fw / (1000.0) << " ms" << endl;
        cout << "Total comms:\t" << (comm_end_fw - comm_start_fw)/(1 << 20) <<endl;
        total_timesteps+=1;
    }
}
...
cout << "Total comms:\t" << (comm_end - comm_start)/(1 << 20) <<endl;
